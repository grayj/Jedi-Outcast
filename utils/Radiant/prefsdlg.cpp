...
#define NUMSNAPSHOT_KEY	  "NumSnapshots"
...
void CPrefsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_USESHADERS, m_bUseShaders);
	DDX_Check(pDX, IDC_CHECK_SHADERTEST, m_bShaderTest);
	DDX_Text(pDX, IDC_EDIT_NUMSNAPSHOTS, m_numSnapShots);
	DDV_MinMaxInt(pDX, m_numSnapShots, 0, 99);
	//}}AFX_DATA_MAP
}
...
void CPrefsDlg::SavePrefs()
{
  ...
  AfxGetApp()->WriteProfileInt(PREF_SECTION, NUMSNAPSHOT_KEY, m_numSnapShots);
  ...
  AfxGetApp()->WriteProfileInt(PREF_SECTION, SHADERS_KEY, m_bUseShaders);
  ...
  AfxGetApp()->WriteProfileInt(PREF_SECTION, SHADERTEST_KEY, m_bShaderTest);
}
